...
function Sidebar() {
    ...
    const navLinks = [
        ...
        },
        {
            ...
            submenu: [
                { label: "Carriers", icon: <Building2 size={20} />, route: "/dashboard/carriers" },
    ...
    const isSubmenuActive = (submenu = []) =>
        submenu.some(sub => activeRoute.startsWith(sub.route));

    useEffect(() => {
        const manageLink = navLinks.find(l => l.submenu);
        if (manageLink && isSubmenuActive(manageLink.submenu)) {
            setOpenSubmenu(true);
        }
    ...
    return (
      ...
      ${isExpanded ? 'w-64' : 'w-24'}`}
        ...
        >
            ...
            {/* Navigation Links */}
            <div className={`flex flex-col w-full gap-y-2 ${isExpanded ? 'px-3' : 'items-center'}`}>
                ...
                ${(activeRoute === link.route || isSubmenuActive(link.submenu)) ? 'bg-blue-600 text-white shadow-md' : ''}`}
                        >
                            <div className="flex items-center gap-x-4">
                                <div className={`transition-colors duration-300 ${(activeRoute === link.route || isSubmenuActive(link.submenu)) ? 'text-white' : 'text-gray-700 group-hover:text-white'}`}>{link.icon}</div>
                                {isExpanded && (
                                    <span className={`text-base font-medium transition-colors duration-300 ${(activeRoute === link.route || isSubmenuActive(link.submenu)) ? 'text-white' : 'text-gray-800 group-hover:text-white'}`}>
                                        {link.label}
                                    </span>
                                )}
                            </div>
                            ...
                            {isExpanded && link.submenu && (
                                openSubmenu ? (
                                    <ChevronDown size={20} className={`transition-colors duration-300 ${(activeRoute === link.route || isSubmenuActive(link.submenu)) ? 'text-white' : 'text-gray-500 group-hover:text-white'}`} />
                                ) : (
                                    <ChevronRight size={20} className={`transition-colors duration-300 ${(activeRoute === link.route || isSubmenuActive(link.submenu)) ? 'text-white' : 'text-gray-500 group-hover:text-white'}`} />
                                )
                            )}
                        </Link>
        ...
        border border-blue-100 rounded-xl p-2
        shadow-inner animate-in fade-in slide-in-from-top-1 duration-300">

                                {link.submenu.map((sub, subIndex) => {
                                    const isActive = activeRoute === sub.route;
